...
void KinovaArm::jointVelocityWithFingerVelocityCallback(const kinova_msgs::JointVelocityWithFingerVelocity::ConstPtr& msg)
{
    // 标准做法：先检查通信层是否停止
    if (!kinova_comm_.isStopped())
    {
        // 1. 解包机械臂关节速度 -> 存入成员变量 joint_velocities_ (类型 AngularInfo)
        joint_velocities_.Actuator1 = msg->joint1;
        joint_velocities_.Actuator2 = msg->joint2;
        joint_velocities_.Actuator3 = msg->joint3;
        joint_velocities_.Actuator4 = msg->joint4;
        joint_velocities_.Actuator5 = msg->joint5;
        joint_velocities_.Actuator6 = msg->joint6;
        joint_velocities_.Actuator7 = msg->joint7;

        // 2. 解包手指速度 -> 存入临时变量 fingers (类型 FingerAngles)
        FingerAngles fingers;
        fingers.Finger1 = msg->finger1;
        fingers.Finger2 = msg->finger2;
        fingers.Finger3 = msg->finger3;

        // 3. 调用我们在第二步中新写的 KinovaComm 接口
